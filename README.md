...
**Reverted: `cpu_period`/`cpu_quota` turned out to be fundamentally incompatible with `cpus:`, not just a config style choice.** The grading benchmark harness (`logs-benchmark-cli`) applies its own platform resource limits by merging a generated override into `docker-compose.yml` that sets the plain `cpus:` field. That merge, tested directly with `docker compose config`, succeeds — `cpu_period`/`cpu_quota` and a merged-in `cpus:` coexist fine at the YAML level. Actually starting the container does not: the Docker Engine daemon rejects it outright, `Error response from daemon: Conflicting options: Nano CPUs and CPU Period cannot both be set`. `cpus:` compiles to NanoCPUs; `cpu_period`/`cpu_quota` populate the older CpuPeriod/CpuQuota fields; the two are mutually exclusive on one container at the engine level, full stop — there is no YAML shape that keeps both. With `cpu_period`/`cpu_quota` in place, the harness's `docker compose up` failed before either container started, meaning the actual scored outcome was **0, not a degraded number** — worse than reverting to the 100ms default ever would have been. This may also explain recent portal submission failures ("could not be finalized cleanly") if the real grading infrastructure applies its limits the same way this harness does.

`docker-compose.yml` now sets plain `cpus: "1"` / `cpus: "0.5"` (same average CPU as before) with no explicit period, i.e. back to Docker's 100ms default — confirmed compatible with the harness's override by actually starting the merged stack, not just validating it. **This knowingly gives up the ~3.5x aggregate-availability / ~74% throughput gain measured above.** It's the right call anyway: a measured performance regression is a worse number than an unmeasured one, but a submission that can't start at all isn't a number, it's a zero. Compatibility with what actually has to run this service comes first; reopening the CFS-period question is only worth doing again if a way is found to express it that the harness's own limit-setting mechanism doesn't collide with.
...
**Update: the 1000ms CFS accounting period referenced throughout this section is no longer the shipped configuration** — reverted for grading-harness compatibility, see Section 6's "Reverted" note. Every number below that's attributed to "1000ms period" was measured honestly, at the time, under that setting; none of it is fabricated or wrong as a historical record. It just no longer describes what `docker-compose.yml` currently ships. The service currently runs at Docker's default 100ms period, which this section's own 100ms-period rows show performing measurably worse on aggregate availability and throughput — that regression is real and current, not just a historical data point.
...
- **Fixed: the aggregate cache's latency used to scale with the number of stored *minute* buckets inside the requested window, not with total row count.** Verified at 1M-row scale (Section 9): the realistic access pattern the load generator actually sends (10-minute window, 1-minute buckets) stayed under 1s (82–235ms), but an artificially wide query (15-day window, 1-hour buckets) measured 1.53s — over the 1s target, because answering it meant scanning every one of ~21,600 stored minute buckets. Fix: `aggregateCache.ts` now also folds every row into hour and day maps alongside the minute map, and `1h`/`1d` requests read the aligned interior of the window straight from the matching coarse map (only the unaligned edges still fall back to the minute map). Re-measured on a fresh 1M-row seed: the same 15-day/1h query dropped to **~30–120ms**; correctness cross-checked byte-exact against `count(*)` SQL ground truth on a fixed window (367,337 == 367,337 == 367,337 across 1h-bucket total, 1d-bucket total, and direct SQL). 17/17 `aggregateCache` unit tests pass (6 new, covering unaligned-edge windows and coarse-bucket pruning).
- **Reverted: the 1000ms CFS accounting period (Section 6) is no longer shipped.** It was a real, measured fix for CPU-throttling-driven aggregate unavailability, but turned out to be fundamentally incompatible with the grading benchmark harness's own resource-limiting mechanism (`cpus:`/NanoCPUs) — Docker's engine refuses to run `cpu_period`/`cpu_quota` and `cpus` on the same container at all, confirmed by actually starting the container, not just validating the Compose YAML. Kept, the harness's own `docker compose up` failed before either container started — a real 0, not a degraded score. `docker-compose.yml` now runs at Docker's default 100ms period; see Section 6 for the full account and the throughput/availability cost this knowingly gives up.
